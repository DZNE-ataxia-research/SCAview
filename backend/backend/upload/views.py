...
def import_pkl(request):
    if request.method == "GET":
        ...
        if request.FILES.get('file'):
            ...
            maxLength = len(df.index)
            print(len(df.index))
            mappings = DatamodelAttributeMapping.objects.all()
            for index, row in df.iterrows():
                print(f"Created {index}/{maxLength} Objects  \r")
                #if index >= 80000:
                    #break
                ...
                i+=1
                    ...
                    #print("HALLO")
                source = DatamodelSource.objects.filter(Abbreviation=row['SOURCE'])
                if not source.exists():
                    source = DatamodelSource.objects.create(Abbreviation=row['SOURCE'], Source=row['SOURCE'])
                else:
                    source = source[0]
                """"
                Variable mapper
                """
                map = mappings.filter(Source_Attribute=row['ATTRIBUTE'])
                if map.exists():
                    map = map[0]
                    attr = map.Target_Attribute.Attribute
                else:
                    attr = row['ATTRIBUTE']
        

                
                data = DataPointsVisit(PID = row['PID'],TIMESTAMP = row['TIMESTAMP'],VALUE = row['VALUE'],ATTRIBUTE = attr,VISIT = row['VISIT'],SOURCE = source)
